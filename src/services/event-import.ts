 ...
 * event is unknown or already imported. The DB writes run in a single
 * transaction so a partial failure leaves no half-cataloged job.
 */
    ...
    if (existing) {
      if (existing.status === "completed") {
        throw AppError.conflict(
          `Event ${eventCode} has already been imported`,
        );
      }
      jobId = existing.id;
      await tx.delete(importFiles).where(eq(importFiles.importJobId, jobId));
    } else {
