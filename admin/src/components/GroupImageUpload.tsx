...
export function GroupImageUpload() {
  ...
  const handleAvatarSave = useCallback(
    async (blob: Blob) => {
      ...
      try {
        ...
        notify("padmakara.groups.avatarUploadFailed", { type: "error" });
      } finally {
  ...
  const handleHeroSave = useCallback(
    async (params: HeroSaveParams) => {
      if (!record?.id || dialog.kind !== "hero") return;
      setSavingHero(true);
      try {
        ...
        notify("padmakara.groups.heroUploadFailed", { type: "error" });
      } finally {
        setSavingHero(false);
      }
    },
  ...
  const heroScale = (record.heroScale as number) ?? 100;
  const heroTransform = heroScale === 100 ? undefined : `scale(${heroScale / 100})`;
  const hasAvatar = !!(record.avatarUrl || record.logoUrl);
  const hasHero = !!record.heroUrl;
  const groupName = (record.nameEn || record.nameEn || "?") as string;
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
          ...
          >
            {avatarDragOver ? (
              ...
              <>
                <img
                  src={(record.avatarUrl || record.logoUrl) as string}
                  alt={groupName}
          ...
          <Box>
            ...
            <Typography variant="caption" display="block" color="text.secondary" sx={{ mt: 0.5 }}>
              {hasAvatar ? "Click image to adjust, or drag & drop a new one" : "Or drag & drop onto the box"}
            </Typography>
            <input
              ref={avatarInputRef}
              type="file"
      ...
      <Box>
        ...
        {hasHero && (
          <Typography variant="caption" color="text.secondary">
            Focal point: {focalX}% / {focalY}% — Zoom: {heroScale}%
          </Typography>
        )}
        <input
          ref={heroInputRef}
